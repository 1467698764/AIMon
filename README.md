...
AIMon 是一个自托管的 AI 中转站渠道监控面板，用来集中管理站点、分组和模型，并持续检查每个模型是否真实可调用。

项目支持 New API、Sub2API、Cloudflare 场景以及不登录站点的手动 API Key 接入。所有配置和测活记录保存在本地 SQLite 中，登录密码与 API Key 会加密存储。
