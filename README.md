...
视觉上采用“克制圆角 + 中性控件 + 短接触阴影”的语言。圆角逐层收敛但整体偏紧：浮层与站点面板 14px，模型卡片 10px，按钮与输入框 8px，标签 4px，胶囊形只留给真正是胶囊的东西——状态徽章、指标条和进度条；仪表盘本身密集且方正，过大的曲率会吃掉每个面板的边角，也会让小控件看起来像药丸。控件一律使用纯色底面加一条发丝描边，不带竖向高光，也不在悬停时上浮：带高光并浮起的渐变按钮是十年前的样式，整条工具栏都这样时会像一排压花塑料件；让控件看起来可按的是它与身后底色的对比，而不是自身的高光。

有框的东西只有两类：站点面板和模型卡片。吸顶工具条和站点目录仍然只用发丝线和留白交代边界——它们是导航，不是内容。站点面板的地板直接用画布色，所以面板是靠一圈边框而不是靠第二层灰底被认出来的，压在上面的卡片也保持原来的对比度。画布本身是纯色的，早先那层顶部 420px 的渐变会在页面上横切出一条明显的接缝，而一块会顺着页面变浅的画布也会破坏层级阶梯——面板地板正是贴着它读的。吸顶工具条用的是**画布色**的毛玻璃而不是面板色：它是一条带子，不是一张卡片，浅色毛玻璃压在画布上时看起来像一张横在标题和第一个站点之间的宽卡片，正是它本不该有的割裂感；标题栏则单独用面板色毛玻璃，与它下方的带子区分开。

按钮有三种底面，靠**颜色**而不是明度区分层级：全屏唯一的品牌色实心主按钮（当前唯一的提交动作），各级“测活”入口共用的品牌浅底（这个工具存在的目的就是测活，它值得和主按钮同一个色系，但不必喊得一样响），以及其余控件共用的中性底面。之前整套控件全部退成灰阶，一屏灰白页面上只剩明度可以区分层级，读起来不像克制，而像没做完。输入型控件（搜索框、下拉、分段控件、文本框）使用比按钮更浅的下沉底面——输入框是一个可以往里打字的洞，按钮是一张可以按下去的面。幽灵按钮（取消 / 返回）不填充；处于开启状态的开关、分段控件里被选中的段、命令面板的键盘光标、被勾选的模型和被选中的常用问题，全部使用同一枚品牌浅底，因此“这一个是选中的”在整个应用里只有一种写法。

配色是一套**暖调的“纸与墨”**：浅色主题的画布是低彩度的沙色，面板与卡片是偏黄的米白，墨色是棕黑而不是蓝黑；深色主题不是中性炭灰而是浓咖色，与浅色是同一族暖调。品牌色是深酒红（浅色 `#7b3355`，深色主题用更亮的一档并配浅粉墨色）。上一版用的是冷灰加靛蓝，那是每一套后台模板的默认长相，铺在一整页小字上读起来发灰发蓝、显得廉价；蓝色被明确排除，而绿 / 黄 / 红三色已经被健康度占用，剩下能读出高级感的就是这支酒红。状态色一并拉进同一族暖调：苔绿而不是翡翠绿，赭黄而不是琥珀黄，砖红而不是火焰红，三种色相属于这一页，而不是三只外挂的信号灯；取的都是能当小字读的那一档，只有指标条上的黄档例外，改用更鲜艳的一级——赭黄暗到能读小字时天生是棕的，当成一条条填充的横条时看起来像旁边绿条边上的一摊泥，而一条横条不必承担字形。状态色仍然只服务两件事：健康度（徽章、逐次圆点、成功分数和指标条用绿 / 黄 / 红 / 灰）和“链接 / 聚焦 / 正在进行”（测活进行中的卡片、任务条、运行状态胶囊和进度条转品牌色）。平台标签这类“事实”保持中性，避免和旁边的健康色抢读。阴影短、紧、偏暖，卡片读起来像一张压在画布上的纸，而不是悬在半空。
