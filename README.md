...
视觉上采用“大圆角 + 柔和渐变 + 悬浮卡片”的语言：圆角逐层收敛，外层面板最圆，内部卡片、控件、标签依次收紧，胶囊形留给状态徽章、分档筛选器和进度条一类“活”的元素。面板与控件都带一层极淡的竖向渐变而不是纯平底色——纯灰的按钮看起来像被禁用，带高光的才像可以按；测活一类主要动作使用带品牌色底纹的强调按钮，唯一的提交动作才使用实心主按钮。模型卡与“详情”弹窗里的每次请求都用顶边一条柔和色带表示状态，替代原先贴在左侧的硬色块，鼠标悬停时卡片轻微上浮。
