...
控件配色是全中性的：底面为白 / 浅灰，图标与文字同为深灰，不带任何蓝色底纹——一整排蓝色调的按钮配蓝色图标，等于把品牌色花在了机壳上，而当每个按钮都带着它时，这一排里就没有哪个更重要了。层级改由底面轻重表达：近黑实心的主按钮（浅色主题近黑、深色主题近白，且全屏只有唯一的提交动作用它）> 灰底强调按钮（各级“测活”入口）> 白底默认控件 > 幽灵按钮（取消 / 返回），处于开启状态的开关用下沉底色加深描边表示。颜色只留给两件事：状态（徽章、成功率条、指标条、健康圆点用绿 / 黄 / 红 / 灰）和“链接 / 聚焦 / 正在进行”（测活进行中的卡片、任务条、运行状态胶囊和进度条才转蓝）。阴影短而贴近，面板读起来像一张压在画布上的纸，而不是悬在半空。模型卡与“详情”弹窗里的每次请求仍用顶边一条柔和色带表示状态，悬停时只加深边框和贴地阴影。
