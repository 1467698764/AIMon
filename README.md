...
- **模型层**是全页唯一保留的卡片，也是这条阶梯里最浅的一层，而且是全页唯一拥有渐变底面的物件：卡内只有三段——第一段是模型名、逐次结果圆点和带成功分数（`2/3`）的状态徽章，第二段把平均首字、平均 TTFT 和平均耗时排成三行，每行自左向右是短标签、数值和指标条，因此不必读完数字就能判断模型快慢，第三段是最近测活时间与四个操作。卡片边框只在整轮失败和测活进行中变色。
...
按钮有三种底面，靠**颜色**而不是明度区分层级：全屏唯一的渐变实心主按钮（当前唯一的提交动作，也是全页唯一允许发光的控件），各级“测活”入口共用的品牌浅底（这个工具存在的目的就是测活，它值得和主按钮同一个色系，但不必喊得一样响），以及其余控件共用的中性底面。之前整套控件全部退成灰阶，一屏灰白页面上只剩明度可以区分层级，读起来不像克制，而像没做完。输入型控件（搜索框、下拉、分段控件、文本框）使用比按钮更浅的下沉底面——输入框是一个可以往里打字的洞，按钮是一张可以按下去的面。幽灵按钮（取消 / 返回）不填充；处于开启状态的开关、分段控件里被选中的段、命令面板的键盘光标、被勾选的模型和被选中的常用问题，全部使用同一枚品牌浅底，因此“这一个是选中的”在整个应用里只有一种写法。

配色是一套**以深色为主的冷调控制台**：深色主题的画布是接近午夜的蓝黑（`#070a16`），面板与卡片沿着同一支蓝往上抬，墨色是偏蓝的白；浅色主题不是纯白办公纸，而是一条**带蓝的冷灰阶梯**：画布 `#e3e8fa`，站点标题带 `#ccd5f0`，分组横条 `#d9dff5`，模型卡片 `#f7f9ff`，纯白只留给必须浮在染色页面之上的浮层。上一版浅色是 `#eceffa` 的画布配纯白卡片：四层声明出来的层级全部落在八个明度点之内，读起来不是四层，而是一整片白蒙蒙的雾，所以这条阶梯被拉开到 `#ccd5f0 → #f7f9ff`，每一档都带彩度，并且没有一块大面积底色是纯白。品牌色是靛蓝（浅色 `#4f46e5`，深色主题换成更亮的一档 `#93a4ff` 并配浅蓝墨色）。上一版试过暖调的“纸与墨”——沙色画布、米白卡片、酒红品牌色，那套配色本身没问题，但它给一台 AI 中转监控台穿了一身古典衣服：这一页要说的是延迟、并发和吞吐，不是纸墨。状态色取三档能当小字读的高饱和：翡翠绿、琥珀黄、玫瑰红，只有指标条上的黄档例外，改用更鲜艳的一级——黄色暗到能读小字时天生是棕的，当成一条条填充的横条时看起来像旁边绿条边上的一摊泥，而一条横条不必承担字形。状态色仍然只服务两件事：健康度（徽章、逐次圆点、成功分数和指标条用绿 / 黄 / 红 / 灰）和“链接 / 聚焦 / 正在进行”（测活进行中的卡片、任务条、运行状态胶囊和进度条转品牌色）。平台标签这类“事实”保持中性，避免和旁边的健康色抢读。阴影短、紧、偏冷，卡片读起来像一块压在画布上的面板；只有主按钮和身份色块额外带一圈品牌色的外发光。（浅色 `#4f46e5`，深色主题换成更亮的一档 `#93a4ff` 并配浅蓝墨色）。上一版试过暖调的“纸与墨”——沙色画布、米白卡片、酒红品牌色，那套配色本身没问题，但它给一台 AI 中转监控台穿了一身古典衣服：这一页要说的是延迟、并发和吞吐，不是纸墨。状态色取三档能当小字读的高饱和：翡翠绿、琥珀黄、玫瑰红，只有指标条上的黄档例外，改用更鲜艳的一级——黄色暗到能读小字时天生是棕的，当成一条条填充的横条时看起来像旁边绿条边上的一摊泥，而一条横条不必承担字形。状态色仍然只服务两件事：健康度（徽章、逐次圆点、成功分数和指标条用绿 / 黄 / 红 / 灰）和“链接 / 聚焦 / 正在进行”（测活进行中的卡片、任务条、运行状态胶囊和进度条转品牌色）。平台标签这类“事实”保持中性，避免和旁边的健康色抢读。阴影短、紧、偏冷，卡片读起来像一块压在画布上的面板；只有主按钮和身份色块额外带一圈品牌色的外发光。

“炫彩”只允许有**一支光谱**，写在令牌里，所有渐变都从它取色：`--grad-brand`（青 → 靛 → 品红，用在承载文字的实心面上，因此三个停靠点都刻意压深，白字才压得住）、`--grad-brand-hover`、`--grad-accent`（纯装饰用的霓虹一档）、`--grad-text`（`background-clip: text`）、`--grad-line`（发丝线）、`--grad-veil`（只有几个百分点的薄雾）、`--grad-wash`（同一支雾的可读强度，给那些否则就是一张平板的大面积底：吸顶工具条、站点面板地板、站点标题带、分组横条和弹窗）、`--grad-card`（模型卡片自己的面，见下）。发出去的地方是一份点名的清单，不是“凡是渐变都能用”：应用标题与页面标题（`渠道监控`）用光谱做字色；三枚身份色块——应用图标、站点图标、登录页锁形图标——用光谱做底并带外发光；全页唯一的主按钮、层级标签和弹窗步骤指示器同样；标题栏、吸顶工具条、站点面板、弹窗、命令面板和登录卡的顶边各是一条渐变发丝线；分组横条的左缘是一道品红到青的竖边。凡是“正在进行”的东西，它的那条边会**扫动**（`aimon-sweep`，背景位置从 0% 走到 200%）：任务条顶边、进度条本身、正在测活的模型卡顶边、搜索框的待检索下划线。反过来，排队中的卡片只把边框染上品牌色而不扫动——排队不是运动。半透明的浅底上不放渐变描边：深色主题的 `--brand-soft` 是半透明的，`border-box` 渐变会从整块底色里透出来，而不是留在那 1px 的边上；这也是“测活”按钮和运行状态胶囊保持纯色浅底的原因。，不是“凡是渐变都能用”：应用标题与页面标题（`渠道监控`）用光谱做字色；三枚身份色块——应用图标、站点图标、登录页锁形图标——用光谱做底并带外发光；全页唯一的主按钮、层级标签和弹窗步骤指示器同样；标题栏、吸顶工具条、站点面板、弹窗、命令面板和登录卡的顶边各是一条渐变发丝线；分组横条的左缘是一道品红到青的竖边。凡是“正在进行”的东西，它的那条边会**扫动**（`aimon-sweep`，背景位置从 0% 走到 200%）：任务条顶边、进度条本身、正在测活的模型卡顶边、搜索框的待检索下划线。反过来，排队中的卡片只把边框染上品牌色而不扫动——排队不是运动。半透明的浅底上不放渐变描边：深色主题的 `--brand-soft` 是半透明的，`border-box` 渐变会从整块底色里透出来，而不是留在那 1px 的边上；这也是“测活”按钮和运行状态胶囊保持纯色浅底的原因。

模型卡片是全页唯一拥有**渐变底面**的物件：`--grad-card` 是沿着这支光谱斜着走的四档——左上角是白（深色主题是近黑），往右下依次染上青、靛、品红，悬停时同一条对角线再深一档。别处的渐变都是一层压在纯色上的薄雾，只有这里是底色本身，因为卡片是这一页真正被读、被比较、被操作的那个东西；一张纯白卡片压在染色的页面上，正是“白蒙蒙”里最响的那一块。除了这张面以外，卡片上的颜色只留在真正表示状态的地方。它原先同时有一条顶边色带、一圈按状态着色的边框、一条着色的成功率带、三个着色的数值和三条着色的指标条：四种颜色的卡框在一屏里同时出现时，健康的多数被画得和唯一那个不再回话的模型一样响，而“一次失败”还会把最安静的“详情”按钮染成整张卡上最红的东西。现在色带去掉，边框只在整轮失败（红）和测活进行中（品牌色）时变色，三个平均值保持墨色、只有落进红档才转红，颜色留给徽章、逐次圆点和指标条本身——一个数值旁边已经有一条替它着色的横条，不必两个都喊。“详情”弹窗里的每次请求仍保留顶边色带：那里一次只读一轮，色带是这一次请求的标签，不是一排卡片的底噪。、一圈按状态着色的边框、一条着色的成功率带、三个着色的数值和三条着色的指标条：四种颜色的卡框在一屏里同时出现时，健康的多数被画得和唯一那个不再回话的模型一样响，而“一次失败”还会把最安静的“详情”按钮染成整张卡上最红的东西。现在色带去掉，边框只在整轮失败（红）和测活进行中（品牌色）时变色，三个平均值保持墨色、只有落进红档才转红，颜色留给徽章、逐次圆点和指标条本身——一个数值旁边已经有一条替它着色的横条，不必两个都喊。“详情”弹窗里的每次请求仍保留顶边色带：那里一次只读一轮，色带是这一次请求的标签，不是一排卡片的底噪。
...
模型卡片的最小宽度是 248px（紧凑密度 220px），目标是让它读起来接近方形：1440px 舒适密度下一张卡是 269×178（约 1.51:1），1280px 紧凑密度下是 229×169。上一版是 324px 宽、136px 高——将近 2.4:1，一整排横条而不是一排瓦片。卡片底部一行放不下时会**换行**而不是压缩：那一行同时有“最近测活”时间、“详情”入口和三个图标按钮，卡片一旦太窄，被截断的一定是时间（`07-30 14:2…`），而这恰好是监控工具最不该省的那个事实。

卡内节奏与卡间距是**两套刻度**：卡片之间 12px，卡片自己的行距 6px，内边距 12px。两者都是 8px 时，卡片看起来没有“里面”，一排卡片读起来像一整块条纹。卡片从六行压到三段：成功分数并进状态徽章（`可用 2/3`），三条通栏的延迟行压成三行紧凑的小统计——每行只有“短标签 + 数值 + 指标条”一行高，作为独立横带时成功率把网格自己的“标签 + 数值”结构又重复了一遍，而三条通栏的行为了三个数字吃掉 99px，标签（“平均首字”）还比它引出的数值更长，长名字现在只留在悬停提示里。三个读数曾经并排成一行三列，那样卡片被拉成 2.4:1 的横条，每条指标条也只剩三分之一张卡的长度——一条那么短的横条说不出话；竖着排回来，卡片才是一块瓦片，每条横条也拿到整个卡内宽度。“详情”按钮上原先写着“N 次失败 / N 次成功”，这个数字已经在徽章里，按钮只需要是一道门。逐次结果圆点可以换行（一轮最多十次，一整行就是 116px，等于三分之一张卡），因此圆点再多也挤不掉徽章。
...
同一色标同时作用于模型卡片上的指标条，以及“详情”弹窗中每一次请求的首字、TTFT 和耗时。卡片上的三个平均值本身保持墨色，只有落进红档才转红：旁边的指标条已经替它着了色。卡片上的标签是短名（首字 / TTFT / 耗时），完整名称在悬停提示里——一个读数只占一行高，标签不该比它引出的数值更长。
