...
分割靠线，不靠留白。整页原先叠了四层框——概览条、工具条卡片、目录卡片、站点面板卡片，里面还有一口下沉的井托着一排带边框的模型卡片——彼此之间是 16–24px 的灰色沟槽，看起来是一堆散落的卡片而不是一块面板。现在只有模型卡片保留完整的边框、底色、圆角和接触阴影，其余容器（吸顶工具条、站点面板、分组标题、站点目录）都只用发丝线和留白交代边界。画布本身也调亮到接近面板底色，两块面板之间就不再出现一条明显的灰色通道。

按钮有三种底面，靠**颜色**而不是明度区分层级：全屏唯一的品牌蓝实心主按钮（当前唯一的提交动作），各级“测活”入口共用的品牌浅底（这个工具存在的目的就是测活，它值得和主按钮同一个色系，但不必喊得一样响），以及其余控件共用的冷调中性底面。之前整套控件全部退成灰阶，一屏灰白页面上只剩明度可以区分层级，读起来不像克制，而像没做完。输入型控件（搜索框、下拉、分段控件、文本框）使用比按钮更浅的下沉底面——输入框是一个可以往里打字的洞，按钮是一张可以按下去的面。幽灵按钮（取消 / 返回）不填充；处于开启状态的开关、分段控件里被选中的段、命令面板的键盘光标、被勾选的模型和被选中的常用问题，全部使用同一枚品牌浅底，因此“这一个是选中的”在整个应用里只有一种写法。

中性底面是**冷调**的，和品牌蓝同一个色温：一屏纯中性灰配黑白文字，只有黑、白、灰三个颜色可用，本身就很难好看，也让页面读起来像未上色的线框图。状态色仍然只服务两件事：健康度（徽章、成功率条、指标条、健康圆点用绿 / 黄 / 红 / 灰）和“链接 / 聚焦 / 正在进行”（测活进行中的卡片、任务条、运行状态胶囊和进度条转蓝）。平台标签这类"事实"保持中性，避免和旁边的健康色抢读。阴影短而贴近，卡片读起来像一张压在画布上的纸，而不是悬在半空。模型卡片与“详情”弹窗里的每次请求都用顶边一条色带表示状态；卡片边框也按同一状态色混入一点，因此一排卡片可以只靠颜色横扫。
