...
视觉上采用“克制圆角 + 纯平底色 + 短接触阴影”的语言。圆角逐层收敛但整体偏紧：最外层面板 14px，卡片 10px，按钮与输入框 8px，标签 4px，胶囊形只留给真正是胶囊的东西——状态徽章、指标条和进度条；仪表盘本身密集且方正，过大的曲率会吃掉每个面板的边角，也会让小控件看起来像药丸。控件一律使用纯色底面加一条发丝描边，不带竖向高光，也不在悬停时上浮：带高光并浮起的渐变按钮是十年前的样式，整条工具栏都这样时会像一排压花塑料件；让控件看起来可按的是它与身后底色的对比，而不是自身的高光。层级由底面强弱表达——实心主按钮（唯一的提交动作）> 品牌色底纹的强调按钮（测活一类主要动作）> 默认控件 > 幽灵按钮（取消 / 返回）。阴影短而贴近，面板读起来像一张压在画布上的纸，而不是悬在半空。模型卡与“详情”弹窗里的每次请求仍用顶边一条柔和色带表示状态，悬停时只加深边框和贴地阴影。
