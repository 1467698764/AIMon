...
- **模型层**是全页唯一保留的卡片：卡内是一张四行三列的网格——第一行是成功率（逐次结果圆点加分数），后三行是平均首字、平均 TTFT 和平均耗时，各自带一条指标条，因此不必读完数字就能判断模型快慢。状态由右上角徽章、逐次圆点和成功率分数承担，卡片边框只在整轮失败时转红。

站点面板、分组标题、吸顶工具条和站点目录都不带边框、底色和阴影，只用标题字号和一条发丝分割线交代层级；相邻分组之间补一条发丝线，说明上一组的卡片到哪里为止。把模型也拉平成表格单元格之后，整页变成一片没有抓手的灰底，所以模型重新回到卡片：一个模型是用户真正要读、要比、要动手的对象。
...
视觉上采用“克制圆角 + 中性控件 + 短接触阴影”的语言。圆角逐层收敛但整体偏紧：浮层 14px，面板 10px，按钮与输入框 8px，标签 4px，胶囊形只留给真正是胶囊的东西——状态徽章、指标条和进度条；仪表盘本身密集且方正，过大的曲率会吃掉每个面板的边角，也会让小控件看起来像药丸。控件一律使用纯色底面加一条发丝描边，不带竖向高光，也不在悬停时上浮：带高光并浮起的渐变按钮是十年前的样式，整条工具栏都这样时会像一排压花塑料件；让控件看起来可按的是它与身后底色的对比，而不是自身的高光。

分割靠线，不靠留白。整页原先叠了四层框——概览条、工具条卡片、目录卡片、站点面板卡片，里面还有一口下沉的井托着一排带边框的模型卡片——彼此之间是 16–24px 的灰色沟槽，看起来是一堆散落的卡片而不是一块面板。现在只有模型卡片保留完整的边框、底色、圆角和接触阴影，其余容器（吸顶工具条、站点面板、分组标题、站点目录）都只用发丝线和留白交代边界。画布只比纯白低一档，卡片压在上面是一层很轻的层次，而不是白纸落在灰底上；画布本身是纯色的，早先那层顶部 420px 的渐变会在无框面板之间横切出一条明显的接缝。吸顶工具条用的是**画布色**的毛玻璃而不是面板色：它是一条带子，不是一张卡片，白色毛玻璃压在灰画布上时看起来像一张横在标题和第一个站点之间的宽卡片，正是它本不该有的割裂感；标题栏则单独用面板色毛玻璃，与它下方的带子区分开。

按钮有三种底面，靠**颜色**而不是明度区分层级：全屏唯一的品牌蓝实心主按钮（当前唯一的提交动作），各级“测活”入口共用的品牌浅底（这个工具存在的目的就是测活，它值得和主按钮同一个色系，但不必喊得一样响），以及其余控件共用的中性底面。之前整套控件全部退成灰阶，一屏灰白页面上只剩明度可以区分层级，读起来不像克制，而像没做完。输入型控件（搜索框、下拉、分段控件、文本框）使用比按钮更浅的下沉底面——输入框是一个可以往里打字的洞，按钮是一张可以按下去的面。幽灵按钮（取消 / 返回）不填充；处于开启状态的开关、分段控件里被选中的段、命令面板的键盘光标、被勾选的模型和被选中的常用问题，全部使用同一枚品牌浅底，因此“这一个是选中的”在整个应用里只有一种写法。

整套颜色改用**开源色板 Radix Colors** 的成品色阶（中性用 slate，品牌用 indigo，状态用 green / amber / red），而不是手调的十六进制值：手调灰阶很难在浅色和深色两套主题里同时保持相邻两级的对比一致，之前那版偏冷的自制灰读起来发脏。状态色取的是可读性色阶（step 11）而不是最鲜艳的那一级，绿 / 黄 / 红并排出现时不会互相刺眼；只有指标条上的黄档例外，改用鲜艳的 step 9——黄色系的 step 11 天生是棕色（它必须暗到能当小字读），当成一条条填充的横条时看起来像旁边绿条边上的一摊泥。状态色仍然只服务两件事：健康度（徽章、逐次圆点、成功率分数和指标条用绿 / 黄 / 红 / 灰）和“链接 / 聚焦 / 正在进行”（测活进行中的卡片、任务条、运行状态胶囊和进度条转蓝）。平台标签这类"事实"保持中性，避免和旁边的健康色抢读。阴影短而贴近，卡片读起来像一张压在画布上的纸，而不是悬在半空。

模型卡片上的颜色只留在真正表示状态的地方。它原先同时有一条顶边色带、一圈按状态着色的边框、一条着色的成功率带、三个着色的数值和三条着色的指标条：四种颜色的卡框在一屏里同时出现时，健康的多数被画得和唯一那个不再回话的模型一样响，而“一次失败”还会把最安静的“详情”按钮染成整张卡上最红的东西。现在色带去掉，边框只在整轮失败（红）和测活进行中（蓝）时变色，三个平均值保持墨色、只有落进红档才转红，颜色留给徽章、逐次圆点、成功率分数和指标条本身——一个数值旁边已经有一条替它着色的横条，不必两个都喊。“详情”弹窗里的每次请求仍保留顶边色带：那里一次只读一轮，色带是这一次请求的标签，不是一排卡片的底噪。

字号与尺寸整体收了一档：标题栏 52px，站点名 17px，模型名与正文 14px，按钮 32px 高、图标按钮 30px，紧凑密度再各降一档。原来的尺寸在 1440px 下一屏只放得下一个站点。
...
卡内节奏与卡间距是**两套刻度**：卡片之间 12px，卡片自己的行距 6px，内边距 12px。两者都是 8px 时，卡片看起来没有“里面”，一排卡片读起来像一整块条纹。成功率也不再是一条单独的着色横带，而是并进指标网格当第一行——作为独立横带时，它把网格自己的“标签 + 数值”结构又重复了一遍，还额外吃掉一圈边框、一层底色和 37px 高度。“详情”按钮上原先写着“N 次失败 / N 次成功”，这个数字在它上方两行就有，去掉之后紧凑密度下的底部一行不再换行。这一版之后 1440px 下卡片从 227px 降到 193px，紧凑密度从 240px 降到 184px。
...
同一色标同时作用于模型卡片上的指标条，以及“详情”弹窗中每一次请求的首字、TTFT 和耗时。卡片上的三个平均值本身保持墨色，只有落进红档才转红：旁边的指标条已经替它着了色。
