...
- **模型层**是全页唯一保留的卡片：顶边一条状态色带，边框也按状态着色，卡内是按成功率着色的成功率条、逐次结果圆点，以及三行各自带指标条的数值，因此不必读完数字就能判断模型快慢，也能只靠颜色横扫一排卡片。
...
整套颜色改用**开源色板 Radix Colors** 的成品色阶（中性用 slate，品牌用 indigo，状态用 green / amber / red），而不是手调的十六进制值：手调灰阶很难在浅色和深色两套主题里同时保持相邻两级的对比一致，之前那版偏冷的自制灰读起来发脏。状态色取的是可读性色阶（step 11）而不是最鲜艳的那一级，绿 / 黄 / 红并排出现时不会互相刺眼。状态色仍然只服务两件事：健康度（徽章、成功率条、指标条、健康圆点用绿 / 黄 / 红 / 灰）和“链接 / 聚焦 / 正在进行”（测活进行中的卡片、任务条、运行状态胶囊和进度条转蓝）。平台标签这类"事实"保持中性，避免和旁边的健康色抢读。阴影短而贴近，卡片读起来像一张压在画布上的纸，而不是悬在半空。模型卡片与“详情”弹窗里的每次请求都用顶边一条色带表示状态；卡片边框也按同一状态色混入一点，因此一排卡片可以只靠颜色横扫。
...
| 模型刷新图标 | 只测活当前模型 |
| 模型复制图标 | 复制模型名称 |
| “N 次失败 / N 次成功 · 详情” | 查看本轮每一次测活的耗时、状态码、错误以及自定义问题的回复原文；测活进行中显示为“第 N/M 次 · 详情”，并随每次请求刷新 |
