...
桌面端左侧站点目录会随页面滚动同步当前站点，并在每个站点右侧显示健康分布；单站模式下点击目录会直接切换并展开目标站点。窄屏把站点目录改为横向选择器，只保留失败数量——站点圆点取最差状态，单看颜色无法区分“一个模型挂了”和“整站挂了”。窄屏下吸顶工具条改为随页滚动，由横向站点选择器承担导航，避免两条吸顶元素争抢同一位置；工具条内的按钮换行显示而不横向滚动，避免按钮文字被截断成半个词。
...
视觉上采用“克制圆角 + 中性控件 + 短接触阴影”的语言。圆角逐层收敛但整体偏紧：最外层面板 14px，卡片 10px，按钮与输入框 8px，标签 4px，胶囊形只留给真正是胶囊的东西——状态徽章、指标条和进度条；仪表盘本身密集且方正，过大的曲率会吃掉每个面板的边角，也会让小控件看起来像药丸。控件一律使用纯色底面加一条发丝描边，不带竖向高光，也不在悬停时上浮：带高光并浮起的渐变按钮是十年前的样式，整条工具栏都这样时会像一排压花塑料件；让控件看起来可按的是它与身后底色的对比，而不是自身的高光。

按钮只有两种底面：全屏唯一的近黑实心主按钮（浅色主题近黑、深色主题近白，只用于当前唯一的提交动作），以及其余全部控件共用的白色默认底面。中间那层灰底“强调按钮”已经取消——白底控件、灰底测活、黑底添加站点三种中性底面挤在同一条工具栏里，读起来是三种没做完的按钮，而不是一个层级。各级“测活”入口保留强调，但改用更重的字重和满强度图标，而不是自己一块底色；输入型控件（搜索框、下拉、分段控件、文本框）统一使用比所在面板低一档的下沉底面，这就是全部底面的集合。幽灵按钮（取消 / 返回）不填充，处于开启状态的开关用下沉底色加深描边表示。

中性灰阶本身也去掉了蓝味：原来的浅灰全部带冷偏（`#f4f7fd`、`#e5ebf6`、`#d3dbe8`），一屏微蓝的机壳看上去就是“一堆蓝按钮”，即使并没有用到品牌色；现在文字可以偏冷，底面一律为纯中性灰。颜色只留给两件事：状态（徽章、成功率条、指标条、健康圆点用绿 / 黄 / 红 / 灰）和“链接 / 聚焦 / 正在进行”（测活进行中的卡片、任务条、运行状态胶囊和进度条才转蓝）。阴影短而贴近，面板读起来像一张压在画布上的纸，而不是悬在半空。模型卡与“详情”弹窗里的每次请求仍用顶边一条柔和色带表示状态，悬停时只加深边框和贴地阴影。
