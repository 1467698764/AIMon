...
- **模型层**使用同一张表格的单元格：顶部是按成功率着色的成功率条和逐次结果圆点，下方三行指标各自带一条长度反映耗时的指标条，数值按色标着色，因此不必读完数字就能判断模型快慢。

同一站点的分组标题和模型单元格全部由发丝分割线切分，不再靠留白隔开：站点面板是画布上的一张纸，而不是装着十几张小卡片的托盘。模型单元格用自身的内阴影画出上边线与左边线，因此任何一条分割线都不会被画两次；网格整体左移 1px，让第一列的竖线藏进面板边框，右侧多出的 1px 由面板裁掉。
...
桌面端左侧站点目录是一条贴在画布上的导轨，不带自己的边框、底色和阴影：一列站点名旁边紧挨着它所导航的面板，再套一层卡片框只会让整页看起来又多了一张卡。目录会随页面滚动同步当前站点，并在每个站点右侧显示健康分布；当前站点是一枚白色纸片，悬停只轻微加深底色。单站模式下点击目录会直接切换并展开目标站点。窄屏把站点目录改为横向选择器，只保留失败数量——站点圆点取最差状态，单看颜色无法区分“一个模型挂了”和“整站挂了”；横向选择器需要吸顶，因此在窄屏下它会重新获得一层底色，避免页面内容从它下面穿过。窄屏下吸顶工具条改为随页滚动，由横向站点选择器承担导航，避免两条吸顶元素争抢同一位置；工具条内的按钮换行显示而不横向滚动，避免按钮文字被截断成半个词。
...
分割靠线，不靠留白。整页原先叠了四层框——概览条、工具条卡片、目录卡片、站点面板卡片，里面还有一口下沉的井托着一排带边框的模型卡片——彼此之间是 16–24px 的灰色沟槽，看起来是一堆散落的卡片而不是一块面板。现在只有真正需要独立成块的东西才有边框：吸顶工具条和站点面板。画布本身也调亮到接近面板底色，两块面板之间就不再出现一条明显的灰色通道。

按钮只有两种底面：全屏唯一的近黑实心主按钮（浅色主题近黑、深色主题近白，只用于当前唯一的提交动作），以及其余全部控件共用的中性灰默认底面。这层灰是刻意的——原来的默认底面是白色，白底按钮压在白色面板、再放到灰白画布上，唯一的标记只剩一条发丝描边，一半控件在悬停之前都读成了标签。各级“测活”入口保留强调，但改用更重的字重和满强度图标，而不是自己一块底色；输入型控件（搜索框、下拉、分段控件、文本框）使用比按钮更浅的下沉底面——输入框是一个可以往里打字的洞，按钮是一张可以按下去的面。幽灵按钮（取消 / 返回）不填充；处于开启状态的开关不新增第三种底面，而是反过来变成一枚浮起的白色纸片，与分段控件里被选中的滑块是同一种关系。

中性灰阶本身也去掉了蓝味：原来的浅灰全部带冷偏（`#f4f7fd`、`#e5ebf6`、`#d3dbe8`），一屏微蓝的机壳看上去就是“一堆蓝按钮”，即使并没有用到品牌色；现在文字可以偏冷，底面一律为纯中性灰。颜色只留给两件事：状态（徽章、成功率条、指标条、健康圆点用绿 / 黄 / 红 / 灰）和“链接 / 聚焦 / 正在进行”（测活进行中的单元格、任务条、运行状态胶囊和进度条才转蓝）。阴影短而贴近，面板读起来像一张压在画布上的纸，而不是悬在半空。模型单元格与“详情”弹窗里的每次请求仍用顶边一条柔和色带表示状态，色带向右迅速淡出，避免一排单元格的顶边连成一条横贯面板的彩色标尺；单元格悬停时只轻微加深底色。
